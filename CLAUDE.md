...
- **Disk vs. manifest.** The asset-manifest walk only globs `.lua`/`.png`/`.json` — never `.xml` — so existing components aren't in `assets.json`. Therefore the editor **reads components by their on-disk gui-tree path** (not manifest-gated), and **create/save register them in `assets.json` ("register-on-save")** to honor the intent that gui files end up in the manifest. **Component basenames are unique tree-wide** (folders organize for humans but don't namespace; `<Component src="x.xml">` resolves by basename anywhere in the tree).
